const fail = (message, details = {}) =>
  Object.assign(new Error(message), { statusCode: 400 }, details);
...
async function roadPlan(
  ...
  const cache = new Map();
  // Au premier echec, les autres workers s'arretent : la tournee est refusee,
  // interroger le geocodeur pour les commandes suivantes ne servait a rien.
  let echec = null;
  const refuser = (order) =>
    fail(
  ...
  async function worker() {
    while (pending.length && !echec) {
      const { order, index } = pending.shift();
      let point = coordinates(order);
      if (!point) {
        // Sans rue, le geocodeur ne rendrait qu'un centre-ville : refuse
        // AVANT de l'appeler.
        if (!order.address) {
          echec = echec || refuser(order);
          return;
        }
        ...
        let matches;
        try {
          matches = await cache.get(address);
        } catch (erreur) {
          echec = echec || erreur;
          return;
        }
      ...
      resolved[index] = { ...order, lat: point.lat, lng: point.lng };
    }
  }
  await Promise.all(
...
function decouperEnTournees(points, depart, max = 50) {
