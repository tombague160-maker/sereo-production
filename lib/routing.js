...
async function roadPlan(
  ...
  const { geocoder = null, arriveeLibre = false } = options;
  // Relecture adverse du lot 6 : `arriveeLibre`, pour une tournee SANS arrivee
  // (creee « sans depart ») -- le plus court CHEMIN OUVERT, comme
  // optimizeOrders (lot 7), et non une boucle qui reviendrait au depart. Le
  // depart tient la place de l'arrivee dans la table, mais y rejoindre
  // l'arrivee coute 0 ; le trace s'arrete au dernier arret, et le troncon
  // « vers l'arrivee » est nul (toujours un troncon par arret, plus un).
  const departure = coordinates(start),
    arrival = arriveeLibre ? departure : coordinates(end);
  ...
  if (!fixedOrder && resolved.length > 1) {
    ...
    if (arriveeLibre) for (const ligne of matrice) ligne[ligne.length - 1] = 0;
  ...
  const result = await osrm(
    `/route/v1/driving/${coords(arriveeLibre ? [departure, ...ordered] : [departure, ...ordered, arrival])}?overview=full&geometries=geojson&steps=false`,
  );
  ...
  const troncons =
    legs.length === ordered.length + (arriveeLibre ? 0 : 1) &&
    legs.every((l) => Number.isFinite(l?.duration) && Number.isFinite(l?.distance))
      ? [
          ...
          ...(arriveeLibre ? [{ duree: 0, distance: 0 }] : []),
        ]
      : null;
  return {
    ordered,
    departure,
    arrival: arriveeLibre ? null : arrival,
...
// Lot 6 (pratique au quotidien) : la table des durees (s) entre des points,
// telle qu'OSRM la rend -- pour inserer une commande urgente au meilleur
// endroit d'une tournee en cours. Meme serveur et meme repli que roadPlan.
async function tableDesDurees(points) {
  const coords = points.map((p) => `${p.lng},${p.lat}`).join(";");
  const table = await osrm(`/table/v1/driving/${coords}?annotations=duration`);
  if (
    table.code !== "Ok" ||
    table.durations?.length !== points.length ||
    table.durations.some((row) => row?.length !== points.length)
  )
    throw fail("Impossible de calculer le trajet routier.");
