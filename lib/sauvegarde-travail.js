...
const { fichiersDeTravail, relireSauvegarde } = require("./sauvegarde-base");
...
async function sauvegarder({ source, destination, tables, maxOctets }) {
  ...
  try {
    ...
    //    (relireSauvegarde : plafond, integrity_check, releve des tables).
    const { comptes, empreintes } = await relireSauvegarde({
      compresse: travail.compresse,
      verification: travail.verification,
      tables,
      maxOctets
    });
