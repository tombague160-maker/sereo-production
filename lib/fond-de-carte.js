...
//                             (absente : la mention d'OSM, et un avertissement)
...
function fondDeCarte(env = process.env) {
  ...
  const attributionDemandee = String(env.SEREO_TUILES_ATTRIBUTION || "").trim();
  const choisi = hote ? {
    url: demande,
    // Sans attribution, le bandeau ne disait plus que « Leaflet » : la licence
    // disparaissait sans un mot. Les fonds courants sont faits des donnees
    // d'OpenStreetMap (ODbL) : c'est leur mention qui s'affiche alors, et le
    // demarrage le signale pour qu'on pose celle du fournisseur.
    attribution: attributionDemandee || OSM.attribution,
  ...
  if (hote && !attributionDemandee && !fondDeCarte.attributionSignalee) {
    fondDeCarte.attributionSignalee = true;
    console.warn("[carte] SEREO_TUILES_ATTRIBUTION absente : la mention d'OpenStreetMap s'affiche a sa place. Pose celle du fournisseur.");
  }
