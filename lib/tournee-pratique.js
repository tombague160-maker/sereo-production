...
function registerTourneePratique(app, deps) {
  const {
    ...
    handleRouteError,
    findClient,
    // Le client tel que /api/clients le rend (24/09) ; sans lui, tel quel.
    sansReleveDImport = client => client,
    addHistory,
    setOrderStatus,
    createStop,
    routeAvecTrace,
    ...
    statutsAPlanifier,
    maxArrets = 50,
    tourneeActiveDeLaCommande,
    nomDeCommande = (o) => String(o?.numero || o?.clientName || o?.id || ""),
    nomDeTournee = () => "",
  } = deps;
  ...
  function reponse(db, routeId, orderId = null, extra = {}) {
    ...
    const trouve = order ? findClient(db, order.clientId) : null;
  ...
  app.post(
    ...
    wrap(async (req, res) => {
      ...
      // Integration de la vague 2 : la definition du lot 2 quand server.js la
      // fournit (tournees actives, brouillon compris ; un arret encore a
      // faire), et un refus qui NOMME la commande et la tournee, comme les
      // autres gardes du lot 2.
      const tourneeQuiLaTient = (db) => (tourneeActiveDeLaCommande
        ? tourneeActiveDeLaCommande(db, orderId)
        : db.routes.find((r) => enCours(r) && r.stops.some((s) => String(s.orderId) === orderId && !estSolde(s))) || null);
      const dejaEnTournee = (db) => {
        const occupee = tourneeQuiLaTient(db);
        if (!occupee) return;
        const commande = db.commandes.find((o) => String(o.id) === orderId);
        const nom = nomDeTournee(occupee);
        throw badRequest(`La commande ${nomDeCommande(commande)} appartient déjà à une tournée active${nom ? ` : « ${nom} »` : ""}.`);
      };
      ...
      // La base se lit table par table, a la demande (24/09). Ce que la suite
      // lit dans l'instantane -- les clients (position d'un arret qui n'a pas
      // la sienne), les reglages (duree d'arret) -- se lit ICI, avant les deux
      // appels au calcul routier : apres, sur un magasin ferme entre-temps
      // (restauration), la route tomberait (revue du 24/09).
      void snapshot.clients;
      void snapshot.settings;
