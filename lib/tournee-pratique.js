 ...
 * A cout egal, la place la plus tot.
 */
function meilleurePlace(nbRestants, cout) {
  let meilleure = 0, meilleurCout = Infinity;
  for (let k = 0; k <= nbRestants; k++) {
...
function registerTourneePratique(app, deps) {
  ...
  app.post(
    ...
    wrap(async (req, res) => {
      ...
      // Sans arrivee enregistree (tournee creee sans depart), on revient au depart,
      // ou, a defaut, a la position : l'optimiseur a besoin d'un bout.
      const arrival = routing.coordinates(route.arrival) ? route.arrival : routing.coordinates(route.departure) ? route.departure : position;
      const plan = await routing.roadPlan(
  ...
  app.post(
    ...
    wrap(async (req, res) => {
      ...
      if (snapshot.routes.some((r) => enCours(r) && r.stops.some((s) => String(s.orderId) === orderId && !estSolde(s))))
      ...
      const k = meilleurePlace(restants.length, cout);
