...
// Les memes, colles a la voie ("3 rue de Dole Bat B Apt 12").
const COMPLEMENT_EN_LIGNE = /\s+(b[aâ]t(iment)?\.?\s*[a-z0-9]{1,3}|apt\.?\s*\d+|app(ar)?t(ement)?\.?\s*\d+|appt\.?\s*\d+|[ée]tage\s*\d+|\d+\s*(er|e|eme|ème)\s*[ée]tage|b\.?\s?p\.?\s*\d+|cs\s*\d+|tsa\s*\d+)\b\.?/gi;
...
function nettoyerAdresse({ rue, codePostal, ville } = {}) {
  ...
  const segments = voie.split(/\s*[,;]\s*/).map(texte).filter(Boolean);
