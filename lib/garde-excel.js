...
// la forme de la production) : c'est le nombre de noeuds qui fait la memoire.
//   - ventes au format Ximi (22 colonnes) : 3 000 lignes (66 000 cellules,
//     ~400 000 noeuds) passent ; 3 500 lignes (77 000 cellules, ~470 000 noeuds)
//     tuent le serveur (« JavaScript heap out of memory ») ;
//   - le fichier du banc de charge v1.14.0 (8 colonnes, texte en ligne) :
//     8 001 lignes (~400 000 noeuds) passent, 9 501 (~475 000) le tuent.
// La garantie de v1.14.0 (5 000 lignes importees, test/api.test.js : 40 008
// cellules, 250 094 noeuds) reste tenue : 45 000 cellules et 280 000 noeuds,
// soit une marge de 1,4 sur le dernier passage mesure et de 1,7 sur la
// premiere mort. Le fichier Ximi actuel : 429 lignes, 9 460 cellules.
//
// Ce que la garde compte, et SEULEMENT cela, est lu (relecture adverse du
// 26/09). La garde lit le repertoire central du zip (la liste qui fait foi,
// celle d'Excel) ; read-excel-file, lui, lit les en-tetes locaux en flux
// (unzipper.Parse) sans jamais le consulter. Une partie presente en en-tete
// local mais absente du repertoire (150 Mo d'espaces dans 151 Ko) passait la
// garde, puis la lecture la decompressait : + 307 Mo. classeurVerifie rend
// donc un classeur RECONSTRUIT des parties decompressees et comptees (non
// compressees, tailles vraies), et c'est lui que le serveur fait lire.
...
function inspecterClasseur(buffer, limites = LIMITES) {
  return inspecter(buffer, limites, null);
}

/**
 * Inspecte le classeur (memes limites, memes refus qu'inspecterClasseur), puis
 * rend un classeur .xlsx (Buffer) fait des seules parties inspectees : celui
 * a donner a read-excel-file, qui ne lit alors rien que la garde n'ait compte.
 */
function classeurVerifie(buffer, limites = LIMITES) {
  const gardees = {};
  inspecter(buffer, limites, gardees);
  const classeur = zipSync(gardees, { level: 0 });
  return Buffer.from(classeur.buffer, classeur.byteOffset, classeur.byteLength);
}

function inspecter(buffer, limites, gardees) {
  ...
  for (const partie of parties) {
    const octets = contenu(buffer, partie, limites.octetsXml - total.octetsXml);
    if (gardees) gardees[partie.nom] = new Uint8Array(octets.buffer, octets.byteOffset, octets.byteLength);
...
module.exports = { inspecterClasseur, classeurVerifie, ClasseurRefuse, LIMITES };
