...
- **Variables CSS dupliquées** dans 3 blocs (`:root`, `@media dark`, `[data-color-scheme="dark"]`). Garder les 3 synchronisés. **`test/e2e/themes.spec.js` le vérifie automatiquement** : il compare les 118 propriétés personnalisées résolues entre le sombre forcé et le sombre système, sans liste codée en dur, et échoue si un bloc diverge. Test prouvé non-vacuous par mutation. La duplication reste structurellement nécessaire (CSS ne permet pas de partager un bloc de déclarations entre une media query et une règle ordinaire), mais elle ne peut plus dériver en silence.
