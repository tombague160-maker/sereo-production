...
function renderImportSummary() {
  ...
  container.innerHTML = `
    <article class="summary-item status-ok">
      ...
      <p>${escapeHtml(accorder(importedCount, "élément traité", "éléments traités"))} le ${escapeHtml(formatDate(importedAt))}.</p>
    </article>
    ${isOrders ? `
      <article class="summary-item ${blocked ? "status-warning" : "status-ok"}">
        <h4>Analyse stock</h4>
        <p>${escapeHtml(accorder(blocked, "commande"))} à corriger ou compléter.</p>
      </article>
      <article class="summary-item status-neutral">
        <h4>Secteurs détectés</h4>
        <p>${escapeHtml(accorder(sectorsCount, "secteur disponible", "secteurs disponibles"))} pour la livraison.</p>
      </article>
    ` : ""}
  `;
}
...
function renderDeliveryCandidates() {
  ...
  if (summary) {
    ...
    summary.textContent = `${accorder(filtered.length, "commande prête", "commandes prêtes")}${dejaPrises} - ${sector}${city}${date}${signalHorsDate()}`;
  }
